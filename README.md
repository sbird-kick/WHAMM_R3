...
**Trace lost when the app traps in a callee (or dynamically).** whamm injects the `wasm:report` flush call before `unreachable` only in the entry function, so a trap there still flushes — but traps inside callees, or dynamic traps (out-of-bounds, div-by-zero), never reach `on_exit` and the recording produces no trace. Wizard's built-in R3 monitor flushes engine-side and is immune. Structural limitation of bytecode rewriting; feature-request material (whamm could at least inject the flush before every statically-visible `unreachable`). Separately, a real whamm bug in this area — report-var writes inside `wasm:report` combined with an opcode probe writing the same report var silently emit an invalid module — has a repro at `../whamm_repro_report_var_invalid_module/`.
