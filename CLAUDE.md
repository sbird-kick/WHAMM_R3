...
**305/305 passing** via `./run_tests.sh` (103 wasm-r3 incl. 4 float-memory + 188 gen_tests + 5 IG multi-module + 9 C/C++). `gen_tests/` holds our own generated/regression tests (committed .wat + .wasm; rebuild: `wat2wasm --debug-names [--enable-multi-memory] x.wat -o x.wasm`), produced 2026-07-23 by a haiku agent fleet across 8 categories (ldst/float/glob/dcall/icall/bulk/grow/mmem). Two quarantine dirs hold correct-but-blocked tests, NOT run by the suite: `gen_tests_oracle_blocked/` (13 tests that crash Wizard's oracle — memory.copy + EC-chain bugs, see `../wizard_r3_oracle_bugs/`) and `gen_tests_whamm_blocked/` (glob_12, blocked on whamm typed-probe interference).
...
| Multi-value IR (2+ results) | whamm bug | Probe with `(res0, res1)` bounds on `call:after` silently never fires (verified: single-value control fires). Repro: `../whamm_repro_multivalue_resn/` (run.sh verified end-to-end) — **report upstream** |
| `wasm:report` cluster | whamm bugs | Verified trio (repro: `../whamm_repro_report_on_trap/`): (a) report-var auto-flush DOES survive traps when no `wasm:report` probe exists; (b) declaring ANY `wasm:report` probe — even empty — suppresses the whole report-var CSV flush, clean exit or not (root cause `src/emitter/rewriting/rules/mod.rs:262`: `"report" => None`); (c) assigning a report var inside `wasm:report` makes `whamm instr` exit 0 but emit an INVALID module (locals mismatch). Our user-lib path (`wasm:report { r3_mem.print_trace(); }`, no report vars) runs at clean exit but NOT on trap — trace lost on trap. **Report upstream** |
