...
> **Re-run 2026-09-21 (after the `r3_mem::print_trace` cursor fix, `ced3c85`):** **1142/1142 PASS**, 0 FAIL, 0 ORDER — 103 wasm-r3 + 894 gen_tests + 115 gen_tests_multi + 5 IG + 11 C/C++ (plus 14 extra multi-module entries the `_host` filter admits).
>
> **`gen_tests_native` (544) was deliberately NOT re-run.** The cursor fix only changes behaviour when `wasm:report` fires more than once, which requires a module whose start function is also an exported entry. A static scan of every module in the repo (`wasm-objdump -x`, checking each start-section index against the export table) found **exactly one module with a start section at all** — `gen_tests/gap_start_fn_exported`, added by that same commit. For all 1671 others `unprinted` is 0 and the emitted slice is the whole trace, byte-identical to the pre-fix code. Skipping the 1.5–3h native phase was a deliberate, reasoned call, not an omission; re-run it before any change that touches report/flush anchoring.
...
generated; **8 reproduce as FAIL**, each re-verified independently of the generating agent.
Full write-up: `gen_candidates/failing/ADVERSARIAL_2026-09-21.md`.

**None is a regression from the cursor fix** — all 8 were re-run with the cursor disabled and
fail identically. Four are new gap-table rows above; three are known gaps that finally have a
reproducing test (cross-memory `memory.copy`, multi-value IR `resN` ordering, and the passive
64KB cap — whose at-boundary control `gen_tests/gap_passive_cap_at_boundary` PASSES, so the
pair brackets the cap exactly).

Treat agent-reported root causes as unverified. The one high-confidence "confirmed" diagnosis
in this batch was **wrong**: the agent attributed `lc_start_and_main_distinct` to whamm
injecting a start section changing wizard's entry policy, but the passing modules get an
injected start section too — the real cause is the report-anchor/entry disagreement above.
The FAIL itself was real; the explanation was not.

**Corpus to adopt next: `wasm-r3-bench`** — 27 real-world replay modules from the Wasm-R3
OOPSLA'24 paper (https://github.com/doehyunbaek/wasm-benchmarks/tree/main/wasm-r3-bench).
Reported to be a drop-in: zero imports, single memory, entry exports bound to `r3 main` /
`r3_main`, which is the convention `test_one.sh --exclude "r3"` already keys on. **Not yet
verified by us.** Also flagged: `wasm-smith` for generated module shapes, and Wizard's own
vendored spec/proposal `.wast` corpus.

## Upstream whamm app corpus (2026-09-21)

First run of our writer against whamm's own `tests/apps` modules (29 .wasm, never previously covered — no overlap with our 1671). Runner classifies by imports: 0 imports -> `test_one.sh`, imports -> `test_c.sh` (`--exclude-imports`).

**25/29 completed: 21 PASS, 2 FAIL (both fixed, see gap table), 1 whamm-blocked, 4 vacuous.**

- **4 vacuous (not passes)**: `add`, `basic`, `cust-page-size`, `no_matched_events` produce zero trace events on both sides, so the comparison proves nothing. `basic` imports `ic0` (dfinity) which wizard can't host.
- **14 of the 21 passes are single-event traces.** These handwritten modules exist to exercise whamm's instrumentation matching, not replay fidelity — thin signal. The substantive passes are `cf` (120 events) and `malloc_init` (9).
- **`mem-grow` is blocked by a live whamm decoder bug**, and it is *our already-known one*: the module is 2-memory doing `memory.grow 1` (non-zero memidx), and whamm rejects it with `Found a memory.* instruction with an invalid reserved byte` — byte-identical to the note in `gen_tests_whamm_blocked/o5_mm_grow_mem1`. **Re-tested against whamm v1.1.0: still broken.** This reproduces on whamm's OWN upstream test app, which is far stronger material for the pending report than our synthetic repro — see `../whamm_repro_multimem_grow/`.
- **4 modules never finished**: `2mm`, `adi` (polybench), `nw` (ostrich), `users` (dfinity/ic0) were still running at ~100% CPU after ~40min (debug-build `whamm instr` plus JVM wizard replay). **Their status is unknown — not counted as passing.** They also straddle the `print_trace` rebuild, so any result they produce is untrustworthy and they need a clean re-run.

This corpus is NOT yet wired into `run_tests.sh`; it lives upstream in `../whamm/tests/apps/`.
...
| Start fn also exported as entry | Our gap | **Fixed 2026-09-21** — when a module's start function is ALSO its exported entry, the engine runs it twice, and whamm injects `@init`/`wasm:report` into the start function, so both fire twice. `print_trace` re-dumped the whole cumulative buffer on each report: 2 recorded events printed as 3 lines (1 then 1+2). Fix: `r3_mem::print_trace` keeps a `printed` cursor and emits only events recorded since the last call. Found by running whamm's own `tests/apps` corpus (`for_numerics`, `for_numerics-skip_ExprFolder`). Regression: `gen_tests/gap_start_fn_exported` (verified to FAIL with the cursor disabled) |
...
| Unexported mutable globals | Our gap — **UNBLOCKED by whamm v1.1.0** (2026-08-05) | Oracle tracks ALL mutable globals (its shadow seeds from live instance values post-instantiation; probes only in non-excluded code — see `wizard-engine/src/monitors/R3Monitor.v3:200`); we track exported+imported only. Widening to all is a one-line filter in `emit_global_probes` — but it triggers a whamm v1.0.0 bug: **two probes on the same opcode with different bound-value types, where the earlier-declared probe reads its bound value into a local (`var x: i32 = arg0;`) → the earlier probe silently never fires** (minimized to 2 probes, no predicates; swapping order moves the failure). Independently opus-audited 2026-07-23: **REAL-BUG** — objdump shows zero bytes injected at the dropped probe's site (silent no-match, exit 0, no diagnostic); predicates don't rescue it; whamm's own `probe-arg-types/two-probes.mm.bk` passes the identical pattern on `call`, so this is a `global.set`-specific untested path (and that `.bk` file is excluded from cargo-test's glob — no active upstream regression guard). Sonnet re-audit 2026-07-25: only the FIRST probe's local declaration gates the drop; static wasm2wat proof of zero injected bytes. Regressed external-call (spurious G;0) and glob-exp-host-mod-multiple (lost G;1;1). **FIXED UPSTREAM in whamm v1.1.0** (issue #334 "type bounds as probe-local shadow var" — the old code let one probe's type bound clobber siblings'; verified fixed against a v1.1.0 build: both counters fire). Repro package deleted per stale-repro rule. ACTION when convenient: bump the toolchain pin to v1.1.0 (worktree at `../whamm-v1.1.0/`, built with rustc 1.94.0 side-by-side — wasmtime 46 needs it), widen `emit_global_probes` to `g.mutable_`, unquarantine glob_12 + o6_12, re-run the full 1671 suite |
...
| whamm at interpreter scale | whamm limit (corrected 2026-07-25) | The suspected separate "giant-function AST-walker overflow" on SQLite was **debunked**: the SIGABRT was the deep-predicate bug (flat 2337-term chain from pre-fix script_gen); a synthetic flat-body sweep to 4M instructions never crashed, and with balanced predicates SQLite instruments crash-free. What remains is **throughput only**: >10min for SQLite (2332 fns), 5.7–9.4 min for QuickJS (debug build) — `gen_candidates/failing/db_sqlite_basic_01`, `js_quickjs_r3`. Lua worked by trimming to 380 fns. **Fold throughput into the deep-predicate filing** |
| Probe bound-var names collide with user globals | whamm bug (2026-07-25, sonnet-audited REAL-BUG) | Declaring any probe pre-registers its `derived_from: null` bound vars as no-location globals (builder_visitor `add_global(loc: None)`); a user global with the same name then dies with `DuplicateIdentifierError` blaming only the user's line. App content irrelevant; wildcards count. Confirmed names: `opname`/`bytecode`/`category_id`/`category_name` (ANY opcode probe), `targets`/`num_targets`/`default_target` (br_table), `align`/`offset`/`memory`/`is_write` (loads/stores), `target_fn_name` (calls), `resolved_fid` (call_ref/table.set variants only). Repro: `../whamm_repro_bound_var_name_collision/` — **report upstream** |
