...
> **Caveat (2026-09-21):** the 1671 figure predates the `r3_mem::print_trace` cursor fix. After that change only **120/120** were re-run — 103 wasm-r3 + 11 C/C++ + 5 IG + the new `gap_start_fn_exported` — all PASS. **`gen_tests` (893), `gen_tests_native` (544) and `gen_tests_multi` (115) have NOT been re-run.** The fix is a no-op when `wasm:report` fires once (`unprinted` is 0, so the emitted slice is the whole trace, exactly as before), which is every previously-passing test, so regression risk is low — but it is unverified, and the full suite still needs a run before 1671 can be re-asserted.
