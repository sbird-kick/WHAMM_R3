...
| `targets` name collision on br_table probes | whamm bug (2026-07-25) | Any global var named `targets` (ANY type) + any `br_table:before/after` probe in the script (incl. via `opcode:*` wildcards) → `DuplicateIdentifierError` blaming the user's declaration line; `br_table` probes carry an implicit compiler param named `targets` (`src/emitter/rewriting/rules/mod.rs:~2028`). Repro: `../whamm_repro_brtable_targets_collision/` (run.sh + ISSUE.md, verified) — **report upstream** |
